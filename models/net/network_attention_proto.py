...
class AttentionPrototype(nn.Module):
    ...
    def forward(self, support):
        # support: [N, K, D, C]
        N, K, D, C = support.shape
        x = support.view(N * K * D, C)
        weights = self.fc(x).view(N, K, D)          # [N, K, D]
        attn = F.softmax(weights.view(N, -1), dim=1).view(N, K, D, 1)
        proto = (attn * support).sum(dim=[1, 2])    # [N, C]
        return proto

# ============ Batch Cosine Similarity Function ============
def batch_cosine(q, proto):
    # q: [Q, D, C], proto: [N, C]
    q_norm = F.normalize(q, dim=-1)          # [Q, D, C]
    proto_norm = F.normalize(proto, dim=-1)  # [N, C]
    sim = torch.einsum('qdc,nc->qdn', q_norm, proto_norm) # [Q, D, N]
    sim = sim.mean(dim=1)                    # [Q, N]
    return sim
...
class FourLayer_64F(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, num_classes=5, neighbor_k=3):
        ...
        self.proto_module = AttentionPrototype(in_dim=64)  # 必须加在__init__内

    def forward(self, input1, input2):
        # 1 查询特征 
        # extract features of input1--query image
        q = self.features(input1)
        #print('查询集q的特征shape为:',q.shape)
        ...
        #print('支持集S的特征shape为:',S.shape)
        # 3. reshape特征：空间展平
        N, K, C, H, W = S.shape
        S_reshape = S.permute(0, 1, 3, 4, 2).contiguous().view(N, K, H*W, C)  # [N, K, 441, 64]
        Q, Cq, Hq, Wq = q.shape
        q_reshape = q.permute(0, 2, 3, 1).contiguous().view(Q, Hq*Wq, Cq)     # [Q, 441, 64]

        # 4. Attention-based prototype
        prototypes = self.proto_module(S_reshape)   # [N, 64]

        # 5. 相似度计算
        similarities = batch_cosine(q_reshape, prototypes)   # [Q, N]
        return similarities
...
class ImgtoClass_Metric(nn.Module):
    ...
    def calculate_weights_matrix(self, matrix):
        y = torch.abs(matrix[:, None, :] - matrix[:, :, None])  # 广播操作
        Z = y.sum(dim=2) + 1e-8 # 防止loss出现nan的情况  + 1e-8
        Q = Z.sum(dim=1, keepdim=True) + 1e-8  # 防止loss出现nan的情况  + 1e-8
        #print('***Z***', Z)
        # print('***Q***', Q)
        weights_matrix = Z / Q
    ...
    def cal_cosinesimilarity(self, input1, input2):
        ...
        for i in range(B):
            ...
            for j in range(len(input2)):
                ...
                # print('topk_value_weighted', topk_value_weighted)

                inner_sim[0, j] = torch.sum(topk_value_weighted)
    ...
    def forward(self, x1, x2):
        ...
        print('Similarity_list-shape为:',Similarity_list.shape)
        print('Similarity_list为:',Similarity_list)
