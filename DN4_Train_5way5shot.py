...
#loss_coefficient = optimized_params['coefficient']
# param1 = optimized_params['param1']
...
parser.add_argument('--dataset_dir', default='./dataset/MSTAR', help='/miniImageNet')
parser.add_argument('--data_name', default='MSTAR', help='miniImageNet|StanfordDog|StanfordCar|CubBird')
...
def calculate_info_nce_loss(support_features, support_labels, query_features, query_labels, temperature=1.0):
    # Create dictionaries to store support features and labels based on different categories
    support_dict = {}
    for label in np.unique(support_labels):
        mask = (support_labels == label)
        support_dict[label] = support_features[mask]

    # Calculate the number of categories
    num_categories = len(support_dict)

    # Initialize the total Info NCE loss
    total_info_nce_loss = 0.0

    # Move data to GPU
    support_features = torch.tensor(support_features, dtype=torch.float32).cuda()
    support_labels = torch.tensor(support_labels, dtype=torch.int64).cuda()
    query_features = torch.tensor(query_features, dtype=torch.float32).cuda()
    query_labels = torch.tensor(query_labels, dtype=torch.int64).cuda()

    # Loop through each query sample
    info_nce_loss_list = []
    for query_feature, query_label in zip(query_features, query_labels):
        # Find the positive samples for this query sample
        positive_samples = support_features[support_labels == query_label]

        # Choose negative samples from other categories
        negative_samples = []
        for label in range(num_categories):
            if label != query_label:
                negative_samples.extend(support_features[support_labels == label])
        # Convert positive and negative samples to tensors and move to GPU
        positive_samples = positive_samples.cuda()
        negative_samples = torch.stack(negative_samples).cuda()
        # Normalize the feature vectors
        positive_samples = torch.nn.functional.normalize(positive_samples, p=2, dim=1)
        negative_samples = torch.nn.functional.normalize(negative_samples, p=2, dim=1)
        query_feature = query_feature.unsqueeze(0)
        query_feature = torch.nn.functional.normalize(query_feature, p=2, dim=1).cuda()
        # Calculate cosine similarities
        pos_similarity = torch.cosine_similarity(query_feature, positive_samples, dim=-1) / temperature
        neg_similarities = torch.cosine_similarity(query_feature, negative_samples, dim=-1) / temperature
        # Calculate numerator and denominator
        numerator = torch.exp(pos_similarity).sum(dim=-1)
        # print('torch.exp(pos_similarity)的shape', torch.exp(pos_similarity).shape)
        # print('numerator的shape', numerator.shape)
        denominator = torch.exp(neg_similarities).sum(dim=-1)
        # print('torch.exp(neg_similarities)的shape', torch.exp(neg_similarities).shape)
        # print('denominator的shape', denominator.shape)
        # Calculate Info NCE loss for this query sample
        info_nce_loss = -torch.log(numerator / (numerator + denominator)).mean()
        # Add the loss to the total Info NCE loss
        info_nce_loss = info_nce_loss.item()
        info_nce_loss_list.append(info_nce_loss)

    return info_nce_loss_list
...
def train(train_loader, model, criterion, optimizer, epoch_index, F_txt):
	...
	for episode_index, (query_images, query_targets, support_images, support_targets) in enumerate(train_loader):
		...
		support_all_images = []
		for ii in support_images:
			for jj in ii:
				support_all_images.append(jj)
		support_all_images = torch.cat(support_all_images, 0).cuda()
		...
		target = target.cuda()
                ...
                task_loss = criterion(output, target.to(torch.int64))
...
def validate(val_loader, model, criterion, epoch_index, best_prec1, F_txt):
	...
	for episode_index, (query_images, query_targets, support_images, support_targets) in enumerate(val_loader):
		...
		target = torch.cat(query_targets, 0)
		target = target.cuda() # len(target) = 75
